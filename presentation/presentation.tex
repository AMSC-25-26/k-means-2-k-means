...
\begin{frame}\frametitle{What is K-means clustering}
    \begin{itemize}
        \item Unsupervised algorithm that partitions data into $k$ clusters
        \item Iterative procedure: initialize $k$ centroids, assign each point to the nearest centroid, update centroids as the mean of assigned points
        \item Objective: minimize within-cluster sum of squares (WCSS):
              $$\sum_{j=1}^k \sum_{x_i \in C_j} \lVert x_i - \mu_j \rVert^2$$
        \item Pros: simple and fast; Cons: requires choosing $k$, sensitive to initialization (see later on this), assumes roughly spherical clusters
    \end{itemize}
\end{frame}
...
\begin{frame}\frametitle{Centroid initializer (K-means++)}
    \begin{itemize}
        \item Implements \texttt{k-means++} : selects initial cluster centroids to improve convergence maximizing inter-centroid distance
        ...
        \item Select next centroid with probability proportional to that squared distance
              ...
              \end{equation}
        \item Repeat weighted sampling until \texttt{k} centroids are chosen
    \end{itemize}
\end{frame}
...
\begin{frame}\frametitle{Benchmarks (iris dataset, $k=3$)}
    ...
    \begin{center}
        \begin{tabular}{r r r r}
            ...
            2        & 1.102324  & 0         & 0.552592   \\
            4        & 1.067402  & 0         & 0.552592   \\
            \hline
        \end{tabular}
    \end{center}
    \vspace{2mm}
    \begin{itemize}
        \item Observed speedup: $\sim 2\times$ from 1 to 2 procs; less dramatical from 2 to 4 due to overhead
    \end{itemize}
\end{frame}
